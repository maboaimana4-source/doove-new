<script lang="ts">
	...
	const playerStyle = $derived.by(() => {
		const vars = [
			...
			resolvedAspectRatio
				? `--doove-player-aspect-ratio: ${resolvedAspectRatio};`
				: "--doove-player-aspect-ratio: 16 / 9;",
			`--doove-player-object-fit: ${objectFit};`,
		];
		return vars.join(" ");
	});
...
<media-controller
	...
	class={`doove-player ${isTheaterMode ? "doove-player-theater" : ""} ${className}`}
	style={playerStyle}
...
>
	...
	<media-loading-indicator class="doove-loading"></media-loading-indicator>

	{#if resolvedBranding?.src}
		{#if resolvedBranding.href}
			<a
				class={`doove-branding ${resolvedBranding.className ?? ""}`}
				href={resolvedBranding.href}
				target="_blank"
				rel="noreferrer"
				aria-label={resolvedBranding.alt}
			>
				<span class="doove-branding-mark">
					<img
				...
				{#if resolvedBranding.name}
					<span class="doove-branding-name">{resolvedBranding.name}</span>
				{/if}
			</a>
		{:else}
			<div
	...
	{#if mergedControls.bigPlay}
		<media-play-button class="doove-big-play" aria-label="Toggle playback">
			<span slot="play" class="doove-icon doove-icon-big">
				<Play class="size-7 translate-x-px" />
			</span>
			<span slot="pause" class="doove-icon doove-icon-big">
				<Pause class="size-7" />
			</span>
		</media-play-button>
	{/if}

	<media-control-bar class="doove-control-bar" noautohide={pinControls ? "" : undefined}>
		<media-play-button class="doove-btn" aria-label="Play or pause">
			<span slot="play" class="doove-icon"><Play class="size-4 translate-x-[0.5px]" /></span>
			<span slot="pause" class="doove-icon"><Pause class="size-4" /></span>
		</media-play-button>

		{#if mergedControls.seek}
			<media-seek-backward-button class="doove-btn" seekoffset="10" aria-label="Back 10 seconds">
				<span slot="icon" class="doove-icon"><RotateCcw class="size-4" /></span>
			</media-seek-backward-button>

			<media-seek-forward-button class="doove-btn" seekoffset="10" aria-label="Forward 10 seconds">
				<span slot="icon" class="doove-icon"><RotateCw class="size-4" /></span>
			</media-seek-forward-button>
		{/if}

		{#if mergedControls.time}
			<media-time-display class="doove-time" showduration></media-time-display>
		{/if}

		<div class="doove-scrubber-wrap">
			<media-time-range class="doove-scrubber" aria-label="Seek">
				{#if thumbnails}
					<media-preview-thumbnail slot="preview" class="doove-thumb"></media-preview-thumbnail>
				{/if}
				<media-preview-time-display slot="preview" class="doove-preview-time"></media-preview-time-display>
			</media-time-range>

			{#if mergedFeatures.markers && markers.length > 0}
				<div class="doove-marker-rail">
					{#each markers as marker (marker.id)}
						{@const markerTooltipId = `marker-${marker.id}`}
						<button
							type="button"
							class="doove-marker"
							style={`left:${markerLeft(marker.time)}%;--doove-marker-color:${markerColor(marker)};`}
							title={marker.label}
						...
						{#if activeTooltipId === markerTooltipId}
							<div
								class="doove-ui-tooltip doove-ui-tooltip-marker"
		...
		{#if mergedControls.volume}
			<media-mute-button class="doove-btn" aria-label="Mute or unmute">
				<span slot="off" class="doove-icon"><VolumeX class="size-4" /></span>
				<span slot="low" class="doove-icon"><Volume class="size-4" /></span>
				<span slot="medium" class="doove-icon"><Volume1 class="size-4" /></span>
				<span slot="high" class="doove-icon"><Volume2 class="size-4" /></span>
			</media-mute-button>
			<media-volume-range class="doove-volume" aria-label="Volume"></media-volume-range>
		{/if}

		{#if mergedControls.playbackRate}
			<media-playback-rate-button
				class="doove-btn doove-btn-text"
		...
		{#if showCaptions}
			<media-captions-button class="doove-btn" aria-label="Captions">
				<span slot="on" class="doove-icon"><Captions class="size-4" /></span>
				<span slot="off" class="doove-icon doove-icon-muted"><Captions class="size-4" /></span>
			</media-captions-button>
		{/if}

		{#if mergedControls.pip}
			<media-pip-button class="doove-btn" aria-label="Picture in picture">
				<span slot="enter" class="doove-icon"><PictureInPicture class="size-4" /></span>
				<span slot="exit" class="doove-icon"><PictureInPicture2 class="size-4" /></span>
			</media-pip-button>
		{/if}

		{#if mergedControls.fullscreen}
			<media-fullscreen-button class="doove-btn" aria-label="Fullscreen">
				<span slot="enter" class="doove-icon"><Maximize class="size-4" /></span>
				<span slot="exit" class="doove-icon"><Minimize class="size-4" /></span>
...
<style>
	:global(.doove-player) {
		display: block;
		width: 100%;
		aspect-ratio: var(--doove-player-aspect-ratio, auto);
		background: #000;
	}

	:global(.doove-player .doove-media) {
		width: 100%;
		height: 100%;
		object-fit: var(--doove-player-object-fit, contain);
		background: #000;
	}
	...
	.doove-branding:hover,
	.doove-branding:focus-visible {
		gap: 10px;
		padding-right: 12px;
		background: rgba(15, 15, 14, 0.5);
	}

	.doove-branding:hover .doove-branding-name,
	.doove-branding:focus-visible .doove-branding-name {
		max-width: 120px;
		opacity: 1;
