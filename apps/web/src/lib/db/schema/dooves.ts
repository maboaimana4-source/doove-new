...
export const dooveSourceEnum = pgEnum("doove_source", ["cloud", "local"]);
 ...
 */
export const dooveStatusEnum = pgEnum("doove_status", [
	"draft",
	"published",
	"archived",
]);

export const doove = pgTable(
	"doove",
	{
		id: text("id").primaryKey(),
		/**
		 * Owning workspace. Dooves are workspace-first — there is no
		...
		source: dooveSourceEnum("source").notNull().default("cloud"),
		status: dooveStatusEnum("status").notNull().default("draft"),
		/** "cloudinary" | "r2" | "s3" | null when local. */
		provider: text("provider"),
		/**
		 * Denormalized last-view timestamp. Updated when a `share_view` row is
		 * inserted for any share of this doove. Indexed for the expiry sweep
		 * (find dooves with no views in N days). NULL = never viewed.
		 */
	...
	(t) => [
		index("doove_workspace_idx").on(t.workspaceId),
		index("doove_workspace_folder_idx").on(t.workspaceId, t.folderId),
		index("doove_workspace_status_idx").on(t.workspaceId, t.status),
		index("doove_owner_idx").on(t.ownerId),
		index("doove_owner_created_idx").on(t.ownerId, t.createdAt),
		// Expiry sweep: WHERE status='published' AND last_viewed_at < now - 14d.
		index("doove_status_last_viewed_idx").on(t.status, t.lastViewedAt),
	],
);

export type Doove = typeof doove.$inferSelect;
