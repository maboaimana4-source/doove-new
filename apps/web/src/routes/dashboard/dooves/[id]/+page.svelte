<script lang="ts">
	...
	import DooveEngagement from "$lib/dashboard/components/DooveEngagement.svelte";
	import DooveShares, { type ShareRow } from "$lib/dashboard/components/DooveShares.svelte";
	...
	const doove = $derived(data.doove);
	...
	const subtitle = $derived(
		`${formatDuration(doove.durationSec)} · ${formatBytes(doove.sizeBytes)} · ${formatRelative(doove.createdAt)}`,
	);

	// Player wants the store-shaped Doove.
	const playerDoove: Doove = $derived({
		id: doove.id,
		title: doove.title,
		durationSec: doove.durationSec,
		createdAt: doove.createdAt,
		sizeBytes: doove.sizeBytes,
		source: doove.source as Doove["source"],
	...
	async function copyLink() {
		try {
			...
			if (!slug) {
				const { slug: newSlug } = await api.shareDoove(doove.id);
				slug = newSlug;
				await invalidateAll();
			}
	...
	async function onPosterPick(e: Event) {
		...
		try {
			await replacePoster(doove.id, file);
...
<PageHeader title={doove.title} {subtitle}>
...
>
	<button
		...
		aria-label="Play {doove.title}"
		class="absolute inset-0 block size-full"
	>
		{#if doove.posterUrl}
			<img src={doove.posterUrl} alt="" class="absolute inset-0 size-full object-cover transition-transform duration-500 group-hover/hero:scale-[1.02]" />
		{/if}
		...
		<span class="absolute left-3 top-3 flex items-center gap-1 rounded-md px-1.5 py-0.5 font-mono text-[10px] font-bold uppercase tracking-wider ring-1 ring-inset backdrop-blur-sm
			{doove.source === 'cloud' ? 'bg-primary/90 text-background ring-primary/40' : 'bg-background/85 text-muted-foreground ring-border-low/50'}">
			{#if doove.source === "cloud"}<Cloud class="size-3" />{doove.provider}{:else}<MonitorPlay class="size-3" />Local{/if}
		</span>
	</button>
...
<div class="mt-6">
	<EngagementHeatmap moments={data.engagement.moments} durationSec={doove.durationSec} />
</div>

<!-- Retention + engagement -->
<div class="mt-6 grid grid-cols-1 gap-4 lg:grid-cols-2">
	<WatchRetention data={retention} />
	<DooveEngagement engagement={data.engagement} />
</div>
...
<!-- Share links -->
<div class="mt-4">
	<DooveShares {shares} creating={creatingShare} onnew={newShare} onrevoke={revokeShare} />
</div>

<!-- Activity feed (this doove) -->
<div class="mt-4">
	<RecentActivity activity={data.activity} limit={12} linkHref={null} />
</div>

{#if playing}
	<PlayerDialog doove={playerDoove} onclose={() => (playing = false)} />
{/if}
