...
import { folder, doove, user, workspaceUsage } from "$lib/db/schema";
...
/** Owner-or-admin gate shared by PATCH and DELETE. Returns the doove row. */
async function authorizeDoove(
	request: Request,
	dooveId: string,
): Promise<{
	id: string;
	...
	if (!isOwner) {
		const [u] = await db
			...
			.limit(1);
		if (u?.role !== "admin") error(403, "Not allowed to modify this doove");
	}
	return row;
}
 ...
 * PATCH /api/dooves/[id]
 *
 * Rename and/or move a doove. Body (only provided keys are written):
 *   - title    : 1–200 chars
...
export const PATCH: RequestHandler = async ({ params, request }) => {
	const row = await authorizeDoove(request, params.id);
	...
	if ("folderId" in body) {
		if (body.folderId === null) {
			...
			// The folder must exist and belong to the doove's workspace —
			// otherwise a user could file a doove into another workspace's tree.
			const [f] = await db
				.select({ id: folder.id })
				.from(folder)
	...
	await db.update(doove).set(patch).where(eq(doove.id, row.id));
	return json({
		ok: true,
...
/**
 * DELETE /api/dooves/[id]
 *
 * Permanently removes a cloud doove: the R2 blob, the row (its shares,
 ...
 * touches the local `.doove`, which remains the source of truth.
 */
export const DELETE: RequestHandler = async ({ params, request }) => {
	...
	if (!isOwner && !isAdmin) error(403, "Not allowed to delete this doove");
	...
	if (row.videoUrl && !/^https?:\/\//.test(row.videoUrl)) {
		await deleteObject(row.videoUrl).catch((err) => {
			console.error(
				`[dooves/delete] blob delete failed for ${row.id} (key=${row.videoUrl}) — row still removed`,
				err,
			);
		});
	}
