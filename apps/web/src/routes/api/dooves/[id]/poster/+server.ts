...
import { doove, user } from "$lib/db/schema";
import {
	deleteObject,
	isStorageConfigured,
	objectKeyFromStored,
 ...
 */
async function authorize(request: Request, dooveId: string) {
...
/**
 * POST /api/dooves/[id]/poster
 *
 * Step 1 of poster replacement: authorize, then return a signed PUT URL bound
 * to a fresh versioned key (`{dooveId}.poster.{version}.webp`). The client
...
export const POST: RequestHandler = async ({ params, request }) => {
	...
	try {
		...
		console.error("[dooves/poster] sign failed", err);
...
/**
 * PUT /api/dooves/[id]/poster
 *
 * Step 2: HEAD-verify the freshly uploaded poster landed, point the doove at
...
export const PUT: RequestHandler = async ({ params, request }) => {
	...
	if (oldKey && oldKey !== key) {
		await deleteObject(oldKey).catch((err) => {
			console.error(
				`[dooves/poster] old poster delete failed for ${row.id} (key=${oldKey}) — new poster still set`,
				err,
			);
		});
	}
