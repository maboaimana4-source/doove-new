...
export const RELEASES: readonly ChangelogRelease[] = [
	...
	},
	{
		...
		date: '2026-05-23',
		changes: [
			{ kind: 'added', summary: 'Inline playback for recordings: tapping a card on the exports page now opens a `PlayerDialog` powered by `@doove/player` (DoovePlayer) with the branded media-chrome controls, instead of jumping straight to the file location. "Show in folder" stays one click away inside the dialog footer.' },
			{ kind: 'added', summary: 'Global `@doove/player/styles.css` import in the desktop root layout so any future inline players pick up the same theming without per-route boilerplate.' },
	...
	},
	{
		...
		changes: [
			...
			{ kind: 'added', summary: 'Web auth foundation: magic-link sign-in and password-reset flows backed by Better Auth + Drizzle, plus a public waitlist endpoint for Doove Cloud.' },
	...
	},
	{
		...
		date: 'Initial beta',
		changes: [
			{ kind: 'changed', summary: 'First public beta of Doove: offline-first desktop screen recorder and editor' },
		],
	},
] as const;
// RELEASES:END
